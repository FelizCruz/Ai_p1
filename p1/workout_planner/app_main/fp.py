    ...
    def find_workout_plan(self, goal_calories, intensity):
        ...
        while frontier:
            ...
            for workout in self.workouts:
                ...
                self.graph.add_node(counter, calories=new_state["calories"], workout=workout["name"])
        ...
        try:
            ...
            if workout_plan:
                ...
                def update(num):
                    ...
                    workout_name = planner.graph.nodes[num].get("workout", "")
                    ax.set_title(f"A* Search Node Expansion (Step {num}) - {workout_name}")

                self.animation = FuncAnimation(fig, update, frames=len(planner.graph.nodes), interval=1000, repeat=False)
                plt.show()
